...
const DoctorAppointments = () => {
  ...
  return (
    ...
    <div className=' bg-white border text-sm max-h-[80vh] overflow-y-scroll min-h-[50vh]'>
      ...
      </div>
      {
        appointments.map((item,index)=>(
          <div className='flex flex-wrap justify-between max-sm:gap-5 max-sm:text-base sm:grid sm:grid-cols-[0.5fr_2fr_1fr_1fr_3fr_1fr_1fr] gap-1 items-center hover:bg-gray-100  text-gray-500 py-3 px-6  border-b' key={index}>
          <p className=' max-sm:hidden'> {index+1}</p>
          <div className='flex items-center gap-2'>
            <img className='w-8 rounded-full' src={item.userData.image}/> <p>{item.userData.name}</p>
          </div>
          <div>
           <p className='text-xs inline border px-2 py-1 border-[#5F6FFF] rounded-full'>{item.payment ? 'Online' : 'Cash'}</p> 
          </div>
          <p className=' max-sm:hidden'>{calculateAge(item.userData.dob)}</p>
          <p>{dateFormat(item.slotDate)}</p>
